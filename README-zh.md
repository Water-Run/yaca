...
> **项目状态（2026-09-16）：Windows 可用预览已在 Server 2008 与真实模型服务上通过验收；正式目标资格待完成。** 目前还没有任何目标发行包通过资格验证。下文分别说明已接通能力和命令 grammar；Win32 x86、Win64 x86_64 与 Linux x86_64 的目标相关行为仍须分别验证。Gate A/B 保持通过，Release Gate R 仍关闭。
...
长期数据根始终是实际 executable 相邻的 `__yaca__`，不随调用者 cwd 漂移。v0.1 没有内建更新器，也不承诺代码签名。现已提供 win32-x86 交叉构建预览包用于初步部署，见 [Windows 首次使用](release/WINDOWS-QUICKSTART.md)。Server 2008 / DeepSeek 的真实旅程见 [最新预览记录](.develope-docs/WINDOWS-PREVIEW-CLOSEOUT-2026-09-16.md)；XP/Win7/CentOS 全套资格及正式发行仍待完成。
