...
1. `--continue` 已把 exact Resolver selection、二次 target verification、同 workspace logical key/filesystem identity、唯一 writer、保守 recovery gate、完整 Config reload 与 Idle Runtime 恢复接到 existing Context core；它恢复 event/config/manifest 和八类 serial 水位而不重放未完成工作。尚未闭合的是 `.context`/select-context 的同路投影，以及跨 workspace 的显式确认/rebind recovery controller。
...
2026-08-30 的受资源门禁串行 suite 为 `418/418`，其中已含 canonical manifest 伪造拒绝、失败前后 Context 字节/代不变、accepted summary + view 原子发布、cache-miss 恢复、手工/自动五类 pending crash bracket/legacy pending 的跨实例恢复、serial/failure-circuit 重建及 production circuit suppression、公开 `.compact` 路由、main/review 自动 preflight 的精确暂停/恢复与失败阻断、Coordinator 可见 STATUS、真实 production owner 成功链、journal ambiguity fail-stop、`--continue` exact target/workspace/writer/recovery/Idle Runtime 路由与 ID 防碰撞，以及 Win32/XP HTTPS patch/source/profile 防漂移、curl standalone/no-option 配置语法、TLS 1.2/双 CA carrier 与 TLS 错误重试矩阵。它证明平台无关 core、同 workspace reopen 路径和交叉构建静态候选，不证明跨 workspace recovery journey 或真实 XP/Win7/CentOS 7 网络与文件系统资格。
...
yaca 的 Permission exact binding、direct filesystem 复核、durable operation、compaction algorithm、Context 原子 publication、pending lifecycle/circuit recovery core、公开 `.compact`/自动 preflight、同 workspace `--continue` 和 trusted component native bridge 已形成底层闭环。当前最大风险转为两类实现接缝和一类资格缺口：`.context`/select-context 等其余公开 action 的 dispatcher/production-port 全枚举、跨 workspace 显式 recovery journey，以及旧系统 native/network/TLS 闭包的真实目标证明。后续应优先消除这些接缝，不再用只到 fake port 或交叉编译的证据外推产品能力。
