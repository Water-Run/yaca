...
- `.model` 已从同一 registry 路由到 draft/production Model owner：旧 CMD 可用无参数逐行 catalog 或 `.model <exact-name>`，不依赖 selector UI、ANSI、Unicode 或补全。production preview 绑定 ConfigGeneration、Context generation/sequence、active ModelView、Prompt/tool/control 与目标定义；保守窗口不兼容在任何变更前 hard reject。
- endpoint/credential/Protocol/usage/Prompt/adapter/streaming/能力边界变化使用唯一 `model-change-N` 默认拒绝确认；确认期间若 Tool approval 成为 pending，未应用的 Model preview 先失效，避免两个模态 owner。确认后完整配置重载仍须与预览的目标定义及 Prompt/network/Permission 环境一致，随后才允许 Context 原子 publication 和 Runtime receipt adoption；TOCTOU 或重复 receipt 均 fail closed。
...
1. `--continue` 与 chat `.context` 已把 exact Resolver selection、二次 target verification、同 workspace logical key/filesystem identity、唯一 writer、保守 recovery gate、完整 Config reload 与 Idle Runtime 恢复接到 existing Context core；它们恢复 event/config/manifest 和八类 serial 水位而不重放未完成工作。`.context` 还以 previewed path/hash → safe old-owner close → exact-hash re-open 关闭名称竞态；尚未闭合的是跨 workspace 的显式确认/rebind recovery controller。
...
2026-08-30 的受资源门禁串行 suite 为 `442/442`，其中已含 canonical manifest 伪造拒绝、失败前后 Context 字节/代不变、accepted summary + view 原子发布、cache-miss 恢复、手工/自动五类 pending crash bracket/legacy pending 的跨实例恢复、serial/failure-circuit 重建及 production circuit suppression、公开 `.compact` 路由、main/review 自动 preflight 的精确暂停/恢复与失败阻断、Coordinator 可见 STATUS、真实 production owner 成功链、journal ambiguity fail-stop、`--continue` exact target/workspace/writer/recovery/Idle Runtime 路由与 ID 防碰撞、`.context` bounded list/two-phase exact-hash switch/fatal race closure、`.details` 最新/精确读取与固定 64 项过期闭包、`.model` 旧 CMD catalog/direct/默认拒绝/Tool approval 模态竞争、生产 ModelView/Prompt/tool/control 窗口预检、Config reload TOCTOU fail-closed 与 next-turn publication 顺序，以及 Win32/XP HTTPS patch/source/profile 防漂移、curl standalone/no-option 配置语法、TLS 1.2/双 CA carrier 与 TLS 错误重试矩阵。它证明平台无关 core、同 workspace reopen/switch 路径和交叉构建静态候选，不证明跨 workspace recovery journey 或真实 XP/Win7/CentOS 7 网络与文件系统资格。
...
yaca 的 Permission exact binding、direct filesystem 复核、durable operation、compaction algorithm、Context 原子 publication、pending lifecycle/circuit recovery core、公开 `.compact`/自动 preflight、同 workspace `--continue`/`.context`、有界 `.details`、turn-boundary `.cautious/.prompt/.model` 和 trusted component native bridge 已形成底层闭环。当前最大风险转为两类实现接缝和一类资格缺口：跨 workspace 显式 recovery journey、尚未接通的有界 `.prompt edit`，以及旧系统 native/network/TLS 闭包的真实目标证明。后续应优先消除这些接缝，不再用只到 fake port 或交叉编译的证据外推产品能力。
