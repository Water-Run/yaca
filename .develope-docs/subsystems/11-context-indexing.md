...
快照复用不违背实时派生，因为它有界、可淘汰且随时可以丢弃，文件树仍是唯一事实源。页大小、最大展开节点数、结果上限和游标格式以后作为配置/实现契约确认。
...
- plain/enhanced 界面对相同语义动作产生相同结果。
...
- plain/enhanced 对同一动作脚本得到相同控制器结果。
...
3. 是否增加显式 `name:`、`hash:`、`path:` 前缀用于消歧。
