...
状态：**手工、自动、durable recovery core 与同 workspace `--continue`/`.context` 完成，其余 controller 和目标校准待闭合** — model-view/summary/atomic-group/admission、no-tool Model port、Context journal、原子 publication、accepted/pending-view 恢复、公开 `.compact`、每次 main/review 请求前 automatic preflight、STATUS/cancel、Runtime receipt gate、跨进程 failure circuit 和 exact reopen/switch 已实现；跨 workspace 显式确认/rebind 与 target calibration 尚待闭合
...
- [x] `.context`/select-context bounded list 与 two-phase exact-hash switch
- [ ] 跨 workspace 显式确认/rebind controller
