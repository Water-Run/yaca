...
当前继续推进 **Windows 首版与 controller 收口**。N1/N2、N3a rebind（`89fee2c`）之后，
已完成 **N3b 原位 import**：只读校验、明确本机映射、精确确认、配置/文件/目录复核，
以及 Session 有效选择、snapshot、审计和 ModelView 的一次性保存。完整 suite **511/511**；
validators **7612/56/553** 与 TP-003/006/008/010、RP-001 全部通过。Windows 远端验证
取消不写、确认期间配置变化拒绝、实际双映射、映射后重开和 busy 拒绝。
见 [N3b 检查点](CONTEXT-IMPORT-2026-09-14.md)。

部署目标仍为 Server 2008（非 R2），按 XP SP3 / win32-x86 基线推进。指定远端实际是
Server 2025，不能替代旧系统验收。Release Gate R 继续关闭；下一项 N3c typed repair。
...
- 受 `.tools/run_with_resource_guard.sh` 保护的完整平台无关 Lua suite 当前为 `511/511`；本轮验证记录见下节。这不是三目标资格证明。

仍缺失或不得宣称完成：

- `--continue` 与同 workspace `.context` 已接通，显式 rebind 已接通，但直接跨 workspace 继续确认尚未开放；三目标 token/资源阈值仍待校准。
- 在线 self-test Stage 2/3 production adapter 仍返回 failed 占位结果；有效配置的 `config-repl` 字段编辑已接通，但任意无效源的交互修复及 Model/Permission 区域管理尚待完成，`context-repl` 已接通 N1/N2、N3a rebind 和 N3b import，N3 typed repair 仍待实现。这些是实现缺口，不能只归入目标资格待办。
