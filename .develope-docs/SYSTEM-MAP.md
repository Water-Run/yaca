...
23. [Web：v0.1 排除 + 双线预留](subsystems/17-web.md)（D-044 排除；D-058 预留 `yaca-web` / `yaca-ie6`）

## 未来 Web 产品线（不在 v0.1 依赖主线内）

- [web-tracks 总览](web-tracks/README.md)
- [yaca-web](web-tracks/yaca-web.md) — 本机本地 Web 主线；服务端 **Java 8**
- [yaca-ie6](web-tracks/yaca-ie6.md) — 有意兼容 IE6 的同族线；服务端 **PHP 5.4**

上述文档仅为预留；实现前须完成 17 号中的实现级重开清单。它们 **不** 插入 v0.1 编码顺序，也 **不** 放宽核心 zip 的零 Web 表面。
...
扩展边界是所有核心系统的约束，不代表 v0.1 实现扩展运行时。Web 在 v0.1 没有配置、CLI、listener、asset、依赖或发行空壳；D-058 只批准设计预留目录与双产品线命名，不批准实现。
