...
2026-09-22：D-072 允许 tools，D-073 进一步取代“每目标一个包/固定五项根目录”：
每目标改为 clean/std/full 三档，clean 仅 yaca，Lua 解释器仍内嵌。
当前构建仍使用旧布局，新增内容须经 F0 的契约/allowlist 同步；其余保证继续有效。
...
## D-071 编码就绪收尾与核心节点推送授权（2026-08-29）

状态：已确认

项目负责人要求按既定 readiness 路线快速完成收尾并准备开始编码，同时明确核心节点推送 `main`。本决定取代 D-070 中仅针对 2026-08-10 离线会话的“不得执行 modern proof”限制，但不改变产品边界或绕过 Gate A/B。

1. **允许**：校正进度真源、收口 W1--W4 owner 规格、建立机器可校验 contract/fixture、执行并归档可丢弃 modern proof、形成全程序实施计划以及完成相应测试工具。
2. **产品实现门**：在 [`ARCHITECTURE-READINESS.md`](ARCHITECTURE-READINESS.md) 的 Gate A 与 [`READINESS-GAP.md`](READINESS-GAP.md) 的 Gate B 通过前，`src/*.lua` 仍不写产品实现；proof/tooling 不能伪装成产品半成品。
3. **技术失败**：若 proof 失败但能在不改变既有用户保证的前提下更换内部技术路线，由技术侧择最保守可证明方案；若必须改变用户保证，仍按 D-070 停止并提交最小 O 决策包。
4. **Git**：继续直接在 `main` 工作；每个完整核心节点提交并推送 `origin/main`。推送前先 fetch、确认无非快进覆盖风险并保持用户已有工作不受影响；不新开 PR、不修改远端保护规则。
5. **核心节点**：至少包括“基线/门禁真源校正”“机器规格与校验闭环”“关键 modern proof 与 Gate A/B 审计”“编码启动计划”。可合并相邻节点，但不得推送明知不一致或无法自检的中间态。

来源：2026-08-29 负责人指示：“按照你的路线推进，快速完成收尾，准备开始编码”；随后补充：“核心节点推送main”。

## D-072 通用 Agent、旧设备兼容与可选工具（2026-09-22）

状态：负责人方向已明确；具体工作包与技术方案见路线，尚未实施或完成机读传播。

负责人本次要求“完整梳理定位”，并明确：

> yaca是便携的 绿色软件 考虑这点. 开箱即用: 包括老旧环境的网络问题.
> 系统可带一个tools目录, 包括打包的便携python 2.7解释器等常用工具.
> 内置lua本身, 和编译的lua同版本, 作为agent可call的工具使用.

> 留意yaca的定位, 更多的不是面向长程开发, 而是老旧设备上的快速问题辅助等.
> 典型的使用情况: 老设备, U盘里装yaca 插上U盘 直接在目录下./yaca.exe 直接可用

负责人随后澄清定位和工具边界，以此解释上述典型场景：

> 不是装在U盘里的的排障助手, 只是说一个典型场景
> 只是通用的Agent, 正好老设备高度兼容, 正好开箱即用
> 系统的tools还包括ssh之类的, 考虑情况进行打包, tools不强制 只是告诉Agent可以用
> 内置的就是lua解释器

随后补充：“说明大改内容, 其实现在除了内置工具外和原来的差别应该不大”。
据此保留原产品定位和主体架构；Lua/可选工具是明确增量，既有兼容性与可靠性按缺口
修补和验收。先前提出的 onedir、新 exec schema、改默认审查、拆模块和格式迁移
属于技术候选，不作为本轮必做项；具体选择仍须由实现必要性与证据支持。

据此归档以下方向：

1. yaca 的定位是通用 Agent，覆盖代码、文件/数据、自动化、系统问题辅助等任务。
   便携、老设备高度兼容和开箱即用是产品特性；U 盘排障是典型场景，不限定产品用途。
   不以长程自主开发需求主导核心复杂度，也不将任务硬性限制为短任务。
2. 绿色运行和旧环境网络可用性是交付条件；指定的 Server 2008 / Cygwin SSH
   环境纳入直接启动的真实验收，不把人工安装运行时或使用桥接命令作为最终体验。
3. 内置可被 Agent 调用、与核心同完整版本的 Lua 解释器；仅有运行核心的 Lua VM
   不足以满足。`tools/` 为可选目录，Python、SSH 等按环境打包，告诉 Agent 可以使用；
   缺少目录或某个可选工具不阻止核心运行，不要求通用注册/插件机制。
   因此 **D-056 的固定五项根布局在此处被部分取代**。
4. 阅读最新 MiniMax Code、ZCode 和 DeepSeek Harness 源码，选择可使核心简洁鲁棒的机制。
   研究其机制不等于采用其语言、插件框架或完整产品范围。
5. 单 Agent、串行工具、邻接数据根、既有 Permission 矩阵、未知副作用不重放及正式
   目标资格门继续有效。新方向不等于授权把未验证包标为正式发布。

完整定位、实施建议和 F0--F6 顺序见
[通用 Agent 路线](PRODUCT-ROADMAP-2026-09-22.md)；源码事实见
[固定提交对照](references/agent-loop-source-review-2026-09-22.md)。
架构、工具边界、迁移、回退和验收建议见[大改蓝图](MAJOR-REDESIGN-PLAN-2026-09-22.md)。
目录细节、工具 schema、终端适配后端、性能门槛及 DoubleCheck 默认值的评估建议
不在本条中标为负责人逐项确认，需随对应工作包形成具体可验证实现。
当前 contracts/manifest 仍描述旧预览；F0 必须先完成所涉及的版本、allowlist 与 fixtures 传播。

本条不新增 Git 远端写授权。本次交付为定位、计划和基线证据，不是新 executable。

## D-073 内嵌 Lua 与每平台三档发行（2026-09-22）

状态：负责人明确的交付方向；具体工具版本为候选，尚未实施或完成发行资格。

负责人原话：

> 解释器还是内嵌到yaca里头的
> tools整理一个默认版本清单. 发行一个平台三个版本: 干净版, 只有yaca; std版本, 包括python 2.x, ssh等基本工具; 以及一个重版本

重版本选择回复：

> 开发工具箱：在 std 上增加 Git、Python 3、编译工具等，按平台兼容性取舍

1. Lua 解释器随 yaca 内嵌，和核心使用同一完整版本。不另发必需的 lua.exe/Lua DLL；
   前述独立 Lua 文件和 onedir 候选由本条取代。
2. 每个平台三个版本：clean 仅 yaca；std 附基本 tools；重版本附开发工具箱。
   重版本暂采用文件名 `full`。原 Win32/Win64/Linux 三目标各三档，共九个计划包。
3. 同一平台三档使用相同核心，只改变工具组合；tools 可以增删，删除后核心及 Lua 仍可用。
4. std 默认含 Python 2.x、SSH 等；full 增加 Git、Python 3、编译工具，允许按目标平台
   选择不同版本/子集，并明确兼容范围，不能隐瞒工具的 OS/CPU 要求。
5. clean 的“仅 yaca”指运行包；配置/历史按原规则在运行后生成。许可、源码、构建与
   测试证据继续交付。机器契约/打包器未更新前，不宣称已有三档发行物。

默认组件及分平台版本见[发行分档清单](../release/TOOL-BUNDLES.md)。
`--lua` 解释器模式、PuTTY/w64devkit 等具体选型是实施建议，不记作负责人指定的 API/品牌。
工具版本须经目标构建与真实运行后锁定。既有 Release Gate R 继续有效，无新增 Git 推送授权。

## D-074 Lua 工具面与 `.ask` 命名（2026-09-22）
...
1. `lua` 是正式内置 Agent 工具；原八个工具增加为九个，不以让模型拼接
   `exec` 命令替代该工具。解释器仍使用同一锁定 Lua 源码，内嵌于 yaca。
2. 输入为 `code`、可选 `args`/`cwd`/`deadline_ms`。脚本经匿名 stdin 传递，
   使用结构化 argv 启动同一 yaca 内层程序，不经过 shell，不落临时脚本。
   `--lua` 保留为解释器入口和手动调用方式。
3. Lua 具有普通进程能力，使用 Shell 权限与同一 DoubleCheck/审批、durable
   intent/result、输出上限、超时、取消及进程树回收；Readonly 不得执行 Lua。
   解释器文件身份和 cwd 一起绑定到审批并在执行前复核。
4. 命令为 `.ask <问题>`，Alt+Enter 不变；界面块名为 ASK。它仍是无工具
   的纯问答，不更改主任务。原 `.side` 命令不再接受。内部命名和数据模型也统一
   使用 ask，按 D-075 的未发布实现处理，不保留迁移和兼容别名。

## D-075 完整 Review 与自主验收（2026-09-22）

负责人明确：

> 实现包括完整的代码Review. 这是干净实现 yaca还没发布 不要考虑过多历史包袱
> 我将离开设备, 自主完成开发和测试验收

1. 实现包含完整代码 Review，覆盖核心 Lua、原生层、入口、网络、工具、持久化、
   权限、交互与发行流程；发现问题须修复并保留验证证据。
2. yaca 尚未发布。实现可直接清理未发布接口和数据模型，不为开发期产物增加
   迁移、旧名别名、重复路径或无必要的兼容层。操作系统兼容底线仍须保持。
3. 继续自主开发、修复和测试验收；使用隔离的测试目录与系统镜像，不破坏现有
   工作数据。候选构建、真实目标运行、完整验收分别记录，不提前宣称可发布。

## D-076 全量文件头与函数、元表注释（2026-09-23）

负责人要求新增并严格执行编码规范，Lua 文件头按 Author、Date、File、Description 的
顺序书写，Author 固定为 WaterRun；任何函数、元表等使用 `--@param`、`--@return`
一类契约注释，并补充强调“注释完整，完全不遗漏”。

1. [编码规范](CODING-STANDARD.md) 同时适用于现有源码、测试、构建和资格验证程序；
   匿名回调、嵌套函数、元方法、原生类型和项目维护的代码模板均包括在内。
2. 文件头使用所在语言的合法注释语法；第三方原始许可和作者声明保留。
3. 全仓语法清点、反例测试和人工语义 Review 共同验收。不能用只检查新代码、添加
   空泛标签或放宽退出码代替补齐；参数、返回值、错误及实际副作用必须与实现一致。
4. 尚未完成的覆盖和 Review 明确记录为待完成，不继承旧候选的注释或发布验收结论。
