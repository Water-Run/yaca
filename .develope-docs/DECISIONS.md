...
## D-072 v0.1 发行资格按实测环境放宽（2026-09-19）

状态：已确认

项目负责人指示："不需要这么严格的约束，WinServer 2008 能跑就很好了"
（2026-09-19，针对 XP SP3 x86 / Win7 SP1 x64 / 裸机 CentOS 7 硬门无法
提供实机的询问）。

1. **v0.1 发行资格**以三个已实测环境为准：win32-x86 于 Windows
   Server 2008 非 R2 x64 实机、win64-x86_64 于 Windows 11 x64 实机、
   linux-x86_64 于 CentOS 7.9.2009（glibc 2.17 / GCC 4.8.5 锁定工具链）
   容器。三环境均有：资格构建、全测试、真实服务商在线验收（Stage 1--3、
   工具/审批/Shell、保存/恢复）与干净机旅程。
2. XP SP3 x86、Windows 7 SP1 x64 实机与裸机 CentOS 7（真实 3.10 内核、
   断电/文件系统矩阵）转为后续版本的资格增强项，不再阻塞 v0.1。
3. 边界如实随证据记录，不在公开声明中夸大：容器共享宿主内核；
   Windows 11 证据不等于 Windows 7 SP1；断电持久性未在真机证明。
4. Release Gate R 依据本决定以独立、可审计的发布提交翻转
   （保留提交名 `docs: publish qualified release evidence`）。
5. 本决定只放宽 v0.1 的目标资格集；D-044 的 v0.1 零表面与其余用户
   保证不变。

来源：2026-09-19 负责人对 C32 硬件请求的答复。取代 D-007/D-056 中
与之冲突的三目标硬门表述。
