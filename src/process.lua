...
Date: 2026-08-29
Author: WaterRun
Description: Builds bounded foreground shell AsyncPorts over native processes.
]]
...
local function sanitize_environment(values, mode, shell_kind)
    ...
    for name, value in pairs(values) do
        ...
        if not FORBIDDEN_ENVIRONMENT_NAMES[comparison_name]
            and (mode == "inherit_filtered" or MINIMAL_ENVIRONMENT_NAMES[comparison_name])
        then
            result[name] = value
        end
    end
...
function M.new(native, options)
    ...
    function service.new_port(spec)
        ...
        local stdout = new_accumulator(validated.output_limit_bytes)
        local stderr = new_accumulator(validated.output_limit_bytes)
        local port = {}

        ---Starts the fixed shell with stdin closed before user code can run.
        -- @param now integer Current monotonic tick.
        -- @return boolean started True after native process creation succeeds.
        function port:start(now)
            ...
            local ok, value = call_native(native, "process_start", request)
        ...
        ---Polls bounded stdout, stderr, and terminal observations once.
        -- @param now integer Current monotonic tick.
        ...
        ---Requests cancellation without claiming that descendants stopped.
        -- @param now integer Current monotonic tick.
        -- @return boolean accepted Whether native cancellation was admitted.
        ...
        ---Joins the process and returns its complete bounded dual-stream result.
        -- @param deadline integer|nil Absolute monotonic deadline.
        -- @return table result Typed process result.
        ...
        ---Releases the native handle after start or join.
        -- @return boolean closed True when the handle is released.
        function port:close()
