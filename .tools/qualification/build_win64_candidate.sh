...
for command in x86_64-w64-mingw32-gcc x86_64-w64-mingw32-objdump \
  x86_64-w64-mingw32-ar x86_64-w64-mingw32-ranlib make patch python3 sha256sum tar git awk realpath; do
...
cp "$SCRIPT_DIR/build_win64_candidate.sh" "$SCRIPT_DIR/windows_package_win64.py" \
...
(
  ...
  env CC="$CC" AR=x86_64-w64-mingw32-ar RANLIB=x86_64-w64-mingw32-ranlib \
    CFLAGS="$CFLAGS" LDFLAGS="$LDFLAGS" \
    "$WORK_ROOT/expat-2.8.2/configure" --host=x86_64-w64-mingw32 \
...
echo "Windows win64 candidate built: $OUTPUT_ROOT"
echo "Real Windows 7 / modern Windows qualification remains pending."
